...
//	GamePlay game = new GamePlay();
	...
	public boolean readFile(String address) {
		try {
			...
			while (!line.trim().isEmpty()) {
				int flag = 0;
				String[] tmp = line.split("\\s+");
				Country c = new Country(Integer.parseInt(tmp[0]), tmp[1], Integer.parseInt(tmp[3]),
						Integer.parseInt(tmp[4]), x, y, 940, 585);
	...
	@Ignore	
	public void testControllerGamePlay() {
...
//		players.add(a);
		con.addPlayers(a);
//		fail("Not yet implemented");
		System.out.println(game.getPlayer().getID());
		playerList = game.getPlayers();
		assertEquals(playerList.firstElement().getID(),"playerNew" );
	}
